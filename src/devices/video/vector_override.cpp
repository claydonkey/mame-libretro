...
float vector_override_options::s_vector_scale_x = 0.0f;
float vector_override_options::s_vector_scale_y = 0.0f;
float vector_override_options::s_vector_offset_x = 0.0f;
float vector_override_options::s_vector_offset_y = 0.0f;
int vector_override_options::s_vector_bright = 0;
int vector_override_options::s_vector_rotate = 0;
char *vector_override_options::vector_override_options::s_vector_port;
...
void vector_override_options::init(emu_options &options)
{
	...
	{ // user specified a scale on the command line
		vector_override_options::s_vector_scale_x = vector_override_options::s_vector_scale_y = scale;
	}
	else
	{ // use the per-axis scales
		vector_override_options::s_vector_scale_x = options.vector_scale_x();
		vector_override_options::s_vector_scale_y = options.vector_scale_y();
	}
	vector_override_options::s_vector_offset_x = options.vector_offset_x();
	vector_override_options::s_vector_offset_y = options.vector_offset_y();
	vector_override_options::s_vector_port = const_cast<char *>(options.vector_port());
	vector_override_options::s_vector_bright = options.vector_bright();
	vector_override_options::s_vector_rotate = options.vector_rotate();
};
...
void vector_override_device::serial_draw_line(float xf0, float yf0, float xf1, float yf1, int intensity)
{
	...
	const int x0 = (xf0 * VECTOR_SERIAL_MAX - VECTOR_SERIAL_MAX / 2) * vector_override_options::s_vector_scale_x + vector_override_options::s_vector_offset_x;
	const int y0 = (yf0 * VECTOR_SERIAL_MAX - VECTOR_SERIAL_MAX / 2) * vector_override_options::s_vector_scale_y + vector_override_options::s_vector_offset_y;
	const int x1 = (xf1 * VECTOR_SERIAL_MAX - VECTOR_SERIAL_MAX / 2) * vector_override_options::s_vector_scale_x + vector_override_options::s_vector_offset_x;
	const int y1 = (yf1 * VECTOR_SERIAL_MAX - VECTOR_SERIAL_MAX / 2) * vector_override_options::s_vector_scale_y + vector_override_options::s_vector_offset_y;

	serial_segment_t *const new_segment = new serial_segment_t(x0, y0, x1, y1, intensity);

	if (m_serial_segments_tail)
		m_serial_segments_tail->next = new_segment;
	else
		m_serial_segments = new_segment;

	m_serial_segments_tail = new_segment;
};
...
int vector_override_device::serial_send()
{
	...
	while (m_serial_segments)
	{
		...
		if (s->intensity > vector_override_options::s_vector_bright)
...
void vector_override_device::serial_draw_point(unsigned x, unsigned y, int intensity)
{
	...
	unsigned bright;
	if (intensity > vector_override_options::s_vector_bright)
		bright = 63;
	else if (intensity <= 0)
		bright = 0;
	else
	...
	if (vector_override_options::s_vector_rotate == 1)
	{
		// +90
		unsigned tmp = x;
	...
	else if (vector_override_options::s_vector_rotate == 2)
	{
		// +180
		x = VECTOR_SERIAL_MAX - x;
		y = VECTOR_SERIAL_MAX - y;
	}
	else if (vector_override_options::s_vector_rotate == 3)
	{
		// -90
		unsigned t = x;
		x = y;
...
void vector_override_device::device_start()
{
	...
	if (filerr)
	{
		fprintf(stderr, "vector_device_st_v: error: osd_file::open failed: %s on port %s\n", const_cast<char *>(filerr.message().c_str()), vector_override_options::s_vector_port);
		::exit(1);
	}
};
